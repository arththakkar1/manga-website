...
export default function SearchPage() {
  ...
  useEffect(() => {
    // Extract search query and tags from the URL
    const pathSegments = pathname.split('/');
    const query = pathSegments[2] || ''; // Assuming the query is at path `/search/[query]/[tags]`
    const tagsFromURL = pathSegments[3]?.split(',') || [];

    setSearchQuery(query);
    setSelectedTags(tagsFromURL);

    if (query || tagsFromURL.length > 0) {
      fetchSearchResults(query, tagsFromURL);
    }
  ...
  const updateURL = (query: string, tags: string[]) => {
    const tagsPath = tags.length > 0 ? `/${tags.join(',')}` : '';
    const newURL = `/search/${query}${tagsPath}`;
